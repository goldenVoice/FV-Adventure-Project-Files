...
public class banana_HeroAttack : MonoBehaviour {
...
//	public GameObject bulletPrefab;
	...
	public GameObject firePoint;		// kung san mag fi fire yung banana.
			...
			if(target != null){
				if(target.transform.position.x < gameObject.transform.position.x){
					...
					if(Time.time - lastShotTime > heroData.fireRate && checkTarget(target) ){
						...
						playSound();
						Shoot(target.GetComponent<Collider2D>(), firePoint);	// function shoot, the targets collider2D is used as parameter
						lastShotTime = Time.time;
					}
...
//					new Vector3 (0,0,1) );
	...
	void Shoot (Collider2D target, GameObject firePosition){
