...
public class HeroAttack_necroMushroom : MonoBehaviour {
			...
			if(target != null){
				if(target.transform.position.x < gameObject.transform.position.x){
					...
					if(Time.time - lastShotTime > heroData.fireRate && checkTarget(target) ){
						...
						Summon(target.GetComponent<Collider2D>(), respawnPoint4);	// function shoot, the targets collider2D is used as parameter
						lastShotTime = Time.time;
					}
