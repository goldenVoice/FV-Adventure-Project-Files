...
public class EnemyAttack_moleCricket : MonoBehaviour {
...
//	public GameObject bulletPrefab;
	...
	moleCricket_burrow moleCricket_burrowScript;
...
//    	anim = (Animator)parent_hero.transform.GetChild(1).GetComponent<Animator>(); 
		...
		moleCricket_burrowScript = transform.parent.GetChild (0).GetComponent<moleCricket_burrow> ();
		justOnce = false;
...
//	 Update is called once per frame
	void Update () {
		...
		if(!justOnce){		// di pa na de deliver ang deadly blow
...
//				}
			...
			if(target != null){
				Debug.Log("this is the " + (Time.time - lastShotTime));
					if(Time.time - lastShotTime > fireRate
			   			&& moleCricket_burrowScript.moleCricketCanAttack ){	// if false to meaning nasa land na sya.
...
//					new Vector3 (0,0,1) );
	...
	void Shoot (Collider2D target){
		 ...
		 EnemyBulletBehavior_moleCricket bulletComp = newBullet.GetComponent<EnemyBulletBehavior_moleCricket>();
