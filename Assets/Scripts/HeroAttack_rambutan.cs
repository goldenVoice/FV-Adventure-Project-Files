...
public class HeroAttack_rambutan : MonoBehaviour {
...
//		Debug.Log ( "statement: " + (Time.time - lastShotTime  ) );
			...
			if (target.transform.position.x < gameObject.transform.position.x) {
				...
				if (Time.time - lastShotTime > heroData.fireRate && checkTarget (target) ) {
					...
					Sting(target.GetComponent<Collider2D>() );	// function shoot, the targets collider2D is used as parameter
					lastShotTime = Time.time;
				}
	...
	void Sting (Collider2D target){
		...
		laser1.enabled = true;
		laser1.SetPosition (0, laser1.transform.position);
		laser1.SetPosition (1, targetPosition);

		laser2.enabled = true;
		laser2.SetPosition (0, laser2.transform.position);
		laser2.SetPosition (1, targetPosition);
