...
//	}
	...
	void Start () {
		...
		if(gameObject.transform.parent.GetChild(6).gameObject.activeSelf){
			int HP_Level = PlayerPrefs.GetInt(currentProfile + gameObject.name + " HP");
			Debug.Log("The HP level: " + HP_Level);
		...
		int FR_Level = PlayerPrefs.GetInt(currentProfile + gameObject.name + " speed"); 	// value to be given to the var fireRate. automatic na mag se set bec. of the upgrade shop
