...
public class EnemyBulletBehavior_snail : MonoBehaviour {
	...
	void Update () {
		...
		if (gameObject.transform.position.Equals (targetPosition)) {
			if (target != null) {
				...
				if(target.name == "Carrot(Clone)" || target.name == "Onion(Clone)" ){
					Debug.Log ("2x the damage! carrot or onion to pare!");
					damage *= 2f;	// twice the damage
				}
